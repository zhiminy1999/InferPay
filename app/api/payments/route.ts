...
export async function GET(request: Request) {
  try {
    ...
    let rows: any[] = []
    if (walletAddress) {
      rows = db.prepare('SELECT * FROM payments WHERE wallet_address = ? ORDER BY timestamp DESC').all(walletAddress)
    } else {
      rows = db.prepare('SELECT * FROM payments ORDER BY timestamp DESC').all()
    }
